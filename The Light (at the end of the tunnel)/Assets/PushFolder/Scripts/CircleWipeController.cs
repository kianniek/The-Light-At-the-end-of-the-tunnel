...
public class CircleWipeController : MonoBehaviour
{
    ...
    {
        StartCoroutine(DoFade(RADIUS, 0f, fadeOutduration, callback));
    }

    public void FadeIn(Action callback = null)
    {
        StartCoroutine(DoFade(0, RADIUS,fadeInduration , callback));
    }

    IEnumerator DoFade(float start, float end, float duration, Action callback = null)
    {
        radius = start;
        UpdateShader();
